...
public class QueryUtils {
    ...
    public static List<News> fetchNewsData(String requestUrl) {
        Log.v(LOG_TAG,"TEST: fetchEarthquakeData() called.");
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                if (tagsArray.length() > 0) {
                    JSONObject tagFirstObject = tagsArray.optJSONObject(0);
                    String firstName = tagFirstObject.optString("firstName");
                    String lastName = tagFirstObject.optString("lastName");
                    author = firstName + " " + lastName;
                }

                News newN = new News(sectionId,webTitle,author,date,time,url);
                news.add(newN);
            }
